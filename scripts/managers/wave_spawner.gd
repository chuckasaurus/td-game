...
## Path2D on which to spawn enemies — assign in the editor.
@export var path: Path2D
## Container node where spawned enemies will be parented (so they're not all
## children of the spawner). Defaults to the spawner itself if unset.
...
func _spawn_one(enemy_data: EnemyData) -> void:
	if enemy_scene == null or path == null:
		push_warning("WaveSpawner missing enemy_scene or path")
		return
	var enemy := enemy_scene.instantiate()
	# Enemy is a PathFollow2D; it must be a child of a Path2D.
